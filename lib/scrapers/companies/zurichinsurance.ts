...
const SOURCE = 'zurichinsurance'
const COMPANY = 'Zurich Insurance'
const UA = 'Mozilla/5.0 (compatible; LehrstellenradarBot/1.0)'
const URL = 'https://www.zurich.com/de-ch/careers/early-careers'

export async function scrapeZurichInsurance(): Promise<RawJob[]> {
  const jobs: RawJob[] = []
  try {
    ...
    $('[class*="job"], [class*="role"], [class*="apprentice"], [class*="card"]').each((_, el) => {
      const $el = $(el)
      ...
      const applyUrl = href.startsWith('http') ? href : `https://www.zurich.com${href}`
