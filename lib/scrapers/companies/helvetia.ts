...
const SOURCE = 'helvetia'
const COMPANY = 'Helvetia'
const UA = 'Mozilla/5.0 (compatible; LehrstellenradarBot/1.0)'
const URL = 'https://www.helvetia.com/ch/web/de/ueber-uns/jobs-und-karriere/lernende.html'

export async function scrapeHelvetia(): Promise<RawJob[]> {
  const jobs: RawJob[] = []
  try {
    ...
    $('[class*="job"], [class*="stelle"], [class*="lern"], [class*="card"]').each((_, el) => {
      ...
      const applyUrl = href.startsWith('http') ? href : `https://www.helvetia.com${href}`
      const externalId = generateExternalId(SOURCE, COMPANY, title, applyUrl)
      jobs.push({ source: SOURCE, company: COMPANY, title, location: '8001 Zürich', applyUrl, externalId })
    })
