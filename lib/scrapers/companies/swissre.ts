...
const SOURCE = 'swissre'
const COMPANY = 'Swiss Re'
const UA = 'Mozilla/5.0 (compatible; LehrstellenradarBot/1.0)'
const URL = 'https://www.swissre.com/careers/students-and-graduates/apprenticeships.html'

export async function scrapeSwissRe(): Promise<RawJob[]> {
  const jobs: RawJob[] = []
  try {
    ...
    $('[class*="job"], [class*="apprentice"], [class*="lern"], [class*="role"], [class*="card"]').each((_, el) => {
      const $el = $(el)
      ...
      const applyUrl = href.startsWith('http') ? href : `https://www.swissre.com${href}`
      const externalId = generateExternalId(SOURCE, COMPANY, title, applyUrl)
      jobs.push({ source: SOURCE, company: COMPANY, title, location: '8022 Zürich', applyUrl, externalId })
    })
    ...
    if (jobs.length === 0) {
      $('a').each((_, el) => {
        ...
        const applyUrl = href.startsWith('http') ? href : `https://www.swissre.com${href}`
        jobs.push({
          source: SOURCE, company: COMPANY, title: text, location: '8022 Zürich',
